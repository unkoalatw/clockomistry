...
function App() {
    ...
    const [selectedZones, setSelectedZones] = useState(() => {
        try {
            ...
            return saved ? JSON.parse(saved) : ['Asia/Taipei', 'America/New_York', 'Europe/London', 'Asia/Tokyo'];
        } catch (e) {
            return ['Asia/Taipei', 'America/New_York', 'Europe/London', 'Asia/Tokyo'];
        }
    });
    ...
    const handleFontUpload = (event) => {
        ...
        if (!file || file.size > 50 * 1024 * 1024) {
            showError('檔案過大 (請小於 50MB)');
            return;
        }
        const reader = new FileReader();
        reader.onload = async (e) => {
            const success = await loadCustomFont(e.target.result);
            if (success) {
                showError('字體匯入成功！(僅本次有效)');
                setFont('custom');
            }
        };
    ...
    return (
        <div
            ...
            className={`min-h-screen w-full flex flex-col items-center justify-center transition-all duration-1000 bg-gradient-to-br ${currentTheme.gradient} ${currentTheme.text} overflow-hidden relative selection:bg-pink-500 selection:text-white`}
        >
            {/* Toast */}
            ...
            {/* Main Card */}
            <div className={`relative z-10 p-8 sm:p-16 rounded-[3rem] transition-all duration-700 ${currentTheme.card} border-t border-l ${isZenMode ? 'scale-110 shadow-none bg-transparent !border-transparent backdrop-blur-0' : ''}`}>
                {mode === 'clock' && (
                    <div className="flex flex-col items-center select-none">
                        <div className="flex items-baseline font-bold tracking-tighter tabular-nums drop-shadow-2xl">
                            <span className="text-[12vw] leading-none">{h}</span>
                            <span className={`text-[12vw] leading-none animate-pulse ${currentTheme.accent}`}>:</span>
                            <span className="text-[12vw] leading-none">{m}</span>
                            <div className="flex flex-col ml-4 justify-end pb-[2vw]">
                                <span className="text-[4vw] opacity-50 font-medium">{s}</span>
                                {showMillis && <span className={`text-[2vw] ${currentTheme.accent} opacity-80`}>{ms}</span>}
                            </div>
                        </div>
                        <div className="mt-4 text-xl sm:text-2xl font-light tracking-[0.3em] opacity-80 uppercase">{formatDate(time)}</div>
                    </div>
                )}

                {mode === 'timer' && (
                    <div className="flex flex-col items-center select-none">
                        <div className="text-[12vw] font-bold tracking-tighter tabular-nums drop-shadow-2xl">
                            {Math.floor(timerSeconds / 60).toString().padStart(2, '0')}:{(timerSeconds % 60).toString().padStart(2, '0')}
                        </div>
                        <div className={`mt-8 flex gap-6 ${isZenMode ? 'opacity-0' : ''}`}>
                            <button onClick={() => setIsTimerRunning(!isTimerRunning)} className={`p-4 rounded-full bg-white/10 backdrop-blur-md border border-white/20`}>
                                {isTimerRunning ? <Pause size={32} /> : <Play size={32} className={currentTheme.accent} />}
                            </button>
                            <button onClick={() => { setIsTimerRunning(false); setTimerSeconds(timerInitial); }} className="p-4 rounded-full bg-white/10 backdrop-blur-md border border-white/20">
                                <RotateCcw size={32} />
                            </button>
                        </div>
                    </div>
                ...
                {mode === 'stopwatch' && (
                    <div className="flex flex-col items-center select-none w-full min-w-[300px]">
                        <div className="text-[10vw] font-bold tracking-tighter tabular-nums flex items-baseline">
                            <span>{stopwatch.m}</span><span className="opacity-50 mx-1">:</span><span>{stopwatch.s}</span>
                            <span className={`text-[5vw] ml-2 ${currentTheme.accent}`}>.{stopwatch.cs}</span>
                        </div>
                        <div className={`mt-8 flex gap-6 ${isZenMode ? 'opacity-0' : ''}`}>
                            <button onClick={() => setIsStopwatchRunning(!isStopwatchRunning)} className="p-4 rounded-full bg-white/10 border border-white/20">
                                {isStopwatchRunning ? <Pause size={32} /> : <Play size={32} className={currentTheme.accent} />}
                            </button>
                            <button onClick={() => { if (isStopwatchRunning) setLaps([stopwatchTime, ...laps]); else { setStopwatchTime(0); setLaps([]); } }} className="p-4 rounded-full bg-white/10 border border-white/20">
